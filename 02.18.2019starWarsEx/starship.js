...
async function arena(ship01, ship02, round = 5) {
    ship01.health = 100;
    ship02.health = 100; // Resetting the health values for each battle
    ship01.shield = 50;
    ship02.shield = 50; // Resetting the shields No. for each battle
...
function statsCalculation(ship01, ship01Result, ship02, ship02Result) {
    ...
    let vikingPoints = Driveshaft.stats.wins * 3 + Driveshaft.stats.draws;
    let babilonPoints = Flywheel.stats.wins * 3 + Flywheel.stats.draws;
    let azeticPoints = Valve.stats.wins * 3 + Valve.stats.draws;
    ...
    for (const ele of pointsArr) {
        if (ele === vikingPoints) {
            finalArr.push(Driveshaft);
        }
        if (ele === babilonPoints) {
            finalArr.push(Flywheel);
        }
        if (ele === azeticPoints) {
            finalArr.push(Valve);
        }
